...
async function test_addLiquidity(tokenA, amountA, tokenB, amountB, caller, to, log) {
  ...
  console.log(
    `\tCaller %s's balance %s %s is greater than %s %s`,
    caller.name,
    weiToEthEn(tokenBBalance0),
    symbolB,
    amountB,
    symbolB
  );
  tokenA = tokenA.connect(caller);
  await tokenA.approve(maker.address, utils.parseEther(amountA.toString()));
  allowance = (await tokenA.allowance(caller.address, maker.address)).toString();
  expectEqual(allowance, utils.parseEther(amountA.toString()));
  console.log(
    "\tCaller %s approved Make %s to spend %s %s.",
    caller.name,
    uiAddr(maker.address),
    amountA,
    symbolA
  );
  if (tokenB != wbnb) {
    tokenB = tokenB.connect(caller);
    await tokenB.approve(maker.address, utils.parseEther(amountB.toString()));
    allowance = (await tokenB.allowance(caller.address, maker.address)).toString();
    expectEqual(allowance, utils.parseEther(amountB.toString()));
    console.log(
      "\tCaller %s approved Make %s to spend %s %s.",
      caller.name,
      uiAddr(maker.address),
  ...
  console.log("\t%s is calling addLiquidityETH to add (%s %s, %s %s) ...".green,
...
const NodeTypes = ["Token", "Maker", "Taker", "Farm", "Repay", "Factory", "XToken"]; // DO not change the order.
...
describe("====================== Stage 1: Deploy contracts ======================\n".yellow, async function () {
  it("Main contracts are deployed.\n".green, async function () {
    ...
    // Maker Deployment.
    maker = await deployMaker(owner, wbnbAddr, wireLibAddr);
    const makerAddr = maker.address;
    consoleLogWithTab(`Maker deployed at: ${maker.address}`);

    // Taker Deployment.
    taker = await deployTaker(owner, wbnbAddr, wireLibAddr);
    ...
    tx = rSyrup.connect(owner).transferOwnership(repayAddr); // Permanent. Irrevocable.
    (await tx).wait();
    ...
    (await tx).wait();
    console.log("\ttaker was wired: maker - O - farm", makerAddr, farmAddr);

    tx = farm.connect(owner).wire(takerAddr, factoryAddr);
    (await tx).wait();
    ...
    await maker.setLiquidityChangeLimit(5000); // set it 5%.

    await taker.setPriceChangeLimit(5000); // set it 5%
    ...
    const feeStores = [
      ...
      "0x03002f489a8D7fb645B7D5273C27f2262E38b3a1",
    ...
    const FeeRates = [
      //(Developer, Buyback, Liquidity, Treasury). Order matters.
...
describe("====================== Stage 2: Test CrossFactory ======================\n".yellow, async function () {
  ...
  it("setFeeTo function was checked.\n".green, async function () {
    ...
    await expectRevertedWith(tx, "Cross: FORBIDDEN");
    consoleLogWithTab("Alice, a non-owner, setting feeTo to Bob reverted with <Cross: FORBIDDEN>");
  ...
  it("setOwner function was checked.\n".green, async function () {
    let tx = factory.connect(alice).setOwner(bob.address);
    await expectRevertedWith(tx, "Cross: FORBIDDEN");
    consoleLogWithTab("Alice, a non-owner, setting feeTo, to Bob, reverted with <Cross: FORBIDDEN>");

    tx = factory.setOwner(bob.address);
    await expectNotReverted(tx);
    consoleLogWithTab("The current owner could hand over the ownership to Bob.");

    tx = factory.connect(bob).setOwner(owner.address);
    await expectNotReverted(tx);
    consoleLogWithTab("Bob, the current owner, could set a new owner.");
  ...
  it("createPair function was checked.\n".green, async function () {
    ...
    await expectRevertedWith(tx, "Cross: IDENTICAL_ADDRESSES");
    consoleLogWithTab("Creating pair with the same two tokens reverted with <Cross: IDENTICAL_ADDRESSES>.");
    ...
    tx = factory.createPair(...tokens);
    await expectRevertedWith(tx, "Cross: PAIR_EXISTS");
    consoleLogWithTab("Creating a pair again with the used tokens reverted with <Cross: PAIR_EXISTS>.");

    tx = factory.createPair(ethers.constants.AddressZero, tokens[0]);
    await expectRevertedWith(tx, "Cross: ZERO_ADDRESS");
    consoleLogWithTab("Creating a new pair with a zero address of token reverted with <Cross: ZERO_ADDRESS>.");
  });
});
...
describe("====================== Stage 3: Test Dex functionalities. ======================\n".yellow,
  async function () {
    ...
    it("1e6 Mock(MCK) tokens were minted to owner.\n".green, async function () {
      ...
      await tokenMint(mock, owner.address, 1e6);
...
async function mintBlocks(blocks) {
  ...
  console.log(`\tminted %s blocks. bn = `.green, blocks, (await ethers.provider.getBlock("latest")).number);
}
...
//        await deposit(0, alice, 10);  // !!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
      ...
      it("WithdrawVest, MassHarvest, MassStake, MassCompound.\n".green, async function () {
        // switchCollectOption(alice, "OffOff");
        // switchCollectOption(bob, "OffOff");
        // switchCollectOption(carol, "OffOff");
        // mintBlocks(100);
