...
contract CrossTaker is Node, ITaker, Ownable, SessionManager, PriceControl, ChainLinkControl {
    ...
    function _swap(
        ...
        for (uint256 i; i < path.length - 1; i++) {
            ...
            (uint256 amount0Out, uint256 amount1Out) = input == pairSnapshot.token0
                ? (uint256(0), amountOut)
                : (amountOut, uint256(0));
            address to = i < path.length - 2 ? CrossLibrary.pairFor(nodes.factory, output, path[i + 2]) : _to;
    ...
    function _captureInitialPairState(address input, address output) internal virtual returns (PairSnapshot memory pairSnapshot, bool isNichePair) {
        pairSnapshot.pair = CrossLibrary.pairFor(nodes.factory, input, output);
    ...
    function swapExactTokensForTokens(
        ...
        TransferHelper.safeTransferFrom(
            path[0],
            msg.sender, 
            CrossLibrary.pairFor(nodes.factory, path[0], path[1]),
            amounts[0]
        );
    ...
    internal virtual returns (uint256 feesPaid) {
        if (sessionParams.isOriginAction && firstPath == nodes.token) {
            feesPaid = _payFeeCrss(payer, principal, feeRates[sessionParams.sessionType], true); // we has not used up the allowance yet.
        }
    }
    ...
    internal virtual {
        if (sessionParams.isOriginAction && path[path.length-1] == nodes.token) {
            ...
            amountOut -= _payFeeCrss(detour, amountOut, rates, false); // we have used up the allowance.
    ...
    function swapExactETHForTokens(
        ...
        amounts = CrossLibrary.getAmountsOut(nodes.factory, msg.value, path);
        require(amounts[amounts.length - 1] >= amountOutMin, sInsufficientOutput);
        IWETH(WETH).deposit{value: amounts[0]}();
        assert(IWETH(WETH).transfer(CrossLibrary.pairFor(nodes.factory, path[0], path[1]), amounts[0]));
    ...
    function swapExactTokensForTokensSupportingFeeOnTransferTokens(
        ...
        TransferHelper.safeTransferFrom(path[0], msg.sender, CrossLibrary.pairFor(nodes.factory, path[0], path[1]), amountIn);
    ...
    function swapExactETHForTokensSupportingFeeOnTransferTokens(
        ...
        assert(IWETH(WETH).transfer(CrossLibrary.pairFor(nodes.factory, path[0], path[1]), amountIn));
        uint256 balanceBefore = IERC20(path[path.length - 1]).balanceOf(to);
