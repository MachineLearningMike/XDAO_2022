...
contract CrossMaker is Node, IMaker, Ownable, SessionManager, LiquidityControl {
  ...
  if (ICrossFactory(nodes.factory).getPair(tokenA, tokenB) == address(0)) {
        ...
        )
    {
        ...
        address pair = CrossLibrary.pairFor(nodes.factory, tokenA, tokenB);
    ...
    function _payTransactionFeeLP(address lp, uint256 principal) internal virtual returns (uint256 feesPaid) {
        if (sessionParams.isOriginAction) { 
            feesPaid = CrossLibrary.payFeeLP(lp, principal, feeRates[sessionParams.sessionType], feeStores);
        }
    }
        ...
        )
    {
        ...
        address pair = CrossLibrary.pairFor(nodes.factory, _token, WETH);
