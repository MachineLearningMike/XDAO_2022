...
// xTGRToken with Governance.
contract XTGRToken is Node, Ownable, IXTGRToken, ERC20 {
    ...
    function safeTGRTransfer(address _to, uint256 _amount) public override {
        require(msg.sender == nodes.farm, "Forbidden");

        uint256 cakeBal = ITGRToken(nodes.token).balanceOf(address(this));
        if (_amount > cakeBal) {
            ITGRToken(nodes.token).transfer(_to, cakeBal);
        } else {
            ITGRToken(nodes.token).transfer(_to, _amount);
        }
    }

    function getOwner() public view override(IXTGRToken, Node) virtual returns (address) {
        return owner();
    }
