...
import "./interfaces/IXDAOFarmTypes.sol";
import "./interfaces/IXDAOFarm.sol";
...
import "./interfaces/IXTGRToken.sol";
import "../core/interfaces/IXDAOPair.sol";
import "./interfaces/ITGRReferral.sol";
...
contract XDAOFarm is Node, IXDAOFarm, BaseRelayRecipient, SessionManager { 
   ...
   constructor(
    ...
    function setTGRReferral(address _crssReferral) external override virtual onlyOwner {
    ...
    * Returns the current pending amount of TGR rewards for a give user.
    ...
    */
    function changeReferrer(address user, address referrer) public override onlyOwner {
        ...
        ITGRReferral(feeParams.crssReferral).recordReferral(user, referrer);
    ...
    */
    ...
    function getSubPooledTGR(uint256 pid, address userAddress) external view validPid(pid) returns(SubPooledTGR memory) {
        return FarmLibrary.getSubPooledTGR(poolInfo[pid], userInfo[pid][userAddress]);
    }
    ...
    */
    ...
    /**
    * @dev Withdraw a given amount of unlocked TGR amount form the user's vesting process.
    */
    ...
    function withdrawVest(uint256 _pid, uint256 _amount) public override validPid(_pid)  returns (uint256 withdrawn) {
        ...
        if (_amount > 0) {
            ...
            FarmLibrary.tolerableTGRTransferFromXTokenAccount(nodes.xToken, msgSender, _amount);
    ...
    */
    ...
    //         compounded = amount;
    //         newLpAmount = FarmLibrary.changeTGRInXTokenToLpInFarm(address(pool.lpToken), nodes, amount, feeParams.treasury);
    ...
    function harvestAccumulated(uint256 _pid) public override virtual validPid(_pid) returns (uint256 harvested) {
        ...
        if (amount > 0) {
            amount -= _payTransactonFee(nodes.token, nodes.xToken, amount, false);
            harvested = amount;
            FarmLibrary.tolerableTGRTransferFromXTokenAccount(nodes.xToken, msgSender, amount);
            user.accumulated = 0;
    ...
    function stakeAccumulated(uint256 _pid) public override virtual validPid(_pid) returns (uint256 staked) {
        ...
        if (amount > 0) {
            ...
            FarmLibrary.tolerableTGRTransferFromXTokenAccount(nodes.xToken, address(this), amount);
    ...
    /**
    * @dev Take all accumulated TGR rewards, across the given list of pool, of the calling user to their wallet.
    */
    ...
    /**
    * @dev Stake all accumulated TGR rewards, accross the given list of pools, of the calling user to the first TGR staking pool.
    */
    function massStakeRewards() external override virtual returns (uint256 rewards) {
        ...
        if (amount > 0) {
            ...
            amount -= FarmLibrary.payDepositFeeTGRFromXTGR(pool, nodes.xToken, amount, feeStores);
            uint256 balance0 = IERC20(nodes.token).balanceOf(address(this));
            FarmLibrary.tolerableTGRTransferFromXTokenAccount(nodes.xToken, address(this), amount);
            amount = IERC20(nodes.token).balanceOf(address(this)) - balance0;
    ...
    internal virtual returns (uint256 feesPaid) {
        ...
        if (sessionParams.isOriginAction && principal > 0) {
            if (address(payerToken) == nodes.token) {
                feesPaid = _payFeeTGR(payerAddress, principal, feeRates[sessionParams.sessionType], fromAllowance);
            } else {
                feesPaid = XDAOLibrary.payFeeLP(payerToken, principal, feeRates[sessionParams.sessionType], feeStores); // payerAddress: address(this).
            }
        }
    }
