...
library CyberSwapLib {
    using SafeMath for uint256;

    // returns sorted token addresses, used to handle return values from pairs sorted in this order
    function sortTokens(address tokenA, address tokenB) internal pure returns (address token0, address token1) {
        require(tokenA != tokenB, "CyberSwapLib: IDENTICAL_ADDRESSES");
        (token0, token1) = tokenA < tokenB ? (tokenA, tokenB) : (tokenB, tokenA);
        require(token0 != address(0), "CyberSwapLib: ZERO_ADDRESS");
    }
    ...
    function quote(
        ...
        require(amountA > 0, "CyberSwapLib: INSUFFICIENT_AMOUNT");
        require(reserveA > 0 && reserveB > 0, "CyberSwapLib: INSUFFICIENT_LIQUIDITY");
    ...
    function getAmountOut(
        ...
        require(amountIn > 0, "CyberSwapLib: INSUFFICIENT_INPUT_AMOUNT");
        require(reserveIn > 0 && reserveOut > 0, "CyberSwapLib: INSUFFICIENT_LIQUIDITY");
    ...
    function getAmountIn(
        ...
        require(amountOut > 0, "CyberSwapLib: INSUFFICIENT_OUTPUT_AMOUNT");
        require(reserveIn > 0 && reserveOut > 0, "CyberSwapLib: INSUFFICIENT_LIQUIDITY");
    ...
    function getAmountsOut(
        ...
        require(path.length >= 2, "CyberSwapLib: INVALID_PATH");
