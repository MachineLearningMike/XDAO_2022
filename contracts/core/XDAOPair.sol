...
import "./interfaces/IXDAOPair.sol";
import "./interfaces/IXDAOFactory.sol";
import "./interfaces/IXDAOCallee.sol";

import "hardhat/console.sol";

contract XDAOPair is IXDAOPair {
    using Math for uint256;
    using SafeMath for uint256;
    using UQ112x112 for uint224;

    string public constant override name = "XDAO LPs";
    string public constant override symbol = "XDAO-LP";
    ...
    function permit(
        ...
        require(deadline >= block.timestamp, "XDAOed deadline");
    ...
    function initialize(address _token0, address _token1) external override {
        require(msg.sender == factory, "XDAO: FORBIDDEN"); // sufficient check
        token0 = _token0;
        token1 = _token1;
    }
    ...
    function _mintFee(uint112 _reserve0, uint112 _reserve1) private returns (bool feeOn) {
        address feeTo = IXDAOFactory(factory).feeTo();
    ...
    function swap(
        ...
        uint256 balance1;
        {
            ...
            if (data.length > 0) IXDAOCallee(to).crossCall(msg.sender, amount0Out, amount1Out, data);
            balance0 = IERC20(_token0).balanceOf(address(this));
            balance1 = IERC20(_token1).balanceOf(address(this));
        }
